...
{
//	pr_info("chardev_buf before read %s.",chardev_buf);

//	pr_info("great news%d!",len_dev_buffer);
	if(*offset >=len_dev_buffer) 
	{
		
		pr_info("offset greater than or equal to len_dev_buffer");
//		*offset=0;
		return 0;
	}
	int len=min((int)length,len_dev_buffer-(*offset));
//	pr_info("the value of len %d the value of offset is %d",len,(int)*offset);


	mutex_lock(&chardev_lock);
	if(copy_to_user(buffer,&chardev_buf[*offset],len))
	{
//		pr_info("error in copy");
...
{
	int space_left=BUF_MAX_LEN-*offset;
		
	int len=min((int)length,space_left);
	if(len <=0)
		return -ENOSPC;
	mutex_lock(&chardev_lock);
	if(copy_from_user(&chardev_buf[*offset],buffer,len))
	{
		
		pr_err("error in copying contents to kernel");
		return -EFAULT;
	}
	chardev_buf[*offset+len]='\0';
	pr_info("copied the message from user space to file %s.",chardev_buf);
	mutex_unlock(&chardev_lock);

	*offset+=len;
	pr_info("the number of bytes written %d",len);
	len_dev_buffer=*offset;
	return len;
